...
# Shared font settings used by every screen.
FONT_MAIN_TITLE = "Geomini 20 bold"
FONT_HEADING = "Geomini 12 bold"
FONT_DEFAULT = "Geomini 10"

# Minimum and maximum bet allowed by each machine.
money_range = {
    "1": {"minimum": 0,
          "maximum": 20},
    "2": {"minimum": 100,
...
# Symbols and payout multipliers for each machine.
machine_winning = {
...
# Weighted random odds for each machine level.
...
# Load the player's saved money, or create the default balance.
try:
...
# Load each machine's saved upgrade level.
machines = {}
# Load the current day number.
try:
...
# Load the number of turns remaining today.
try:
...
# Load the remaining debt that the player must repay to win.
try:
...
# Check whether the first-playthrough tutorial has been completed.
try:
...
try:
    with open("tutorial_complete.txt", "r") as f:
        tutorial_complete = f.read() == "1"
except FileNotFoundError:
    tutorial_complete = False

# Game rules, validation, persistence, and state updates.
class Logic:

    def __init__(self, root):
    # Store the window and references to GUI status variables.
        self.root = root
        self.day_var = None
        self.turn_var = None
        self.debt_var = None
    ...
    def get_input(self, entry):
        # Convert entry-box text into a whole number.
        try:
            entry = int(entry)
            return entry
        except ValueError:
    ...
    def check_input(self, entry, machine_number):
        # Check that a bet is affordable and within the machine's range.
        global player_money
    ...
    def save_player_money(self):
        # Persist the player's current balance.
        with open("player_money.txt", "w") as f:
                f.write(str(player_money))

    def save_debt(self):
        # Persist the player's remaining debt.
        global debt
        with open("debt.txt", "w") as f:
            f.write(str(debt))
    ...
    def upgrade_cost(machine_number):
        # Calculate the next upgrade price from machine number and level.
        return (100 * int(machine_number) ** 2) * machines[machine_number] ** 3

    def pay_debt(self, entry, currency_var):
        # Validate and process a debt payment.
        global player_money
        global debt
    ...
    def configure_slots(self, slot1, slot2, slot3, machine_number):
        # Choose weighted symbols and reveal them with a short animation.
        global machines
    ...
    def roll(self, entry, currency_var, machine_number, slot1, slot2, slot3, button):
        # Process a bet, animate the result, and award winnings.
        global player_money
        global machines
    ...
    def upgrade(self, currency_var, machine_number, odds_label=None, level_label=None, cost_label=None):
        # Purchase a machine upgrade and refresh its displayed statistics.
        global player_money
        global machines
        player_money = float(player_money)
    ...
    def machine_level(self, label, machine_number):
        # Display the machine level or MAX when fully upgraded.
        global machines
    ...
    def working(self, currency_var):
        # Give the player random income and spend one turn.
        global player_money
        player_money = float(player_money)
    ...
    def odd_lbl_config(self, label, machine_number):
        # Build and display the selected machine's odds table.
        global machines
        upgrade = str(machines[machine_number])
        string = ""
    ...
    def progress_day(self):
        # Advance the day and restore the daily turn allowance.
        global day
        global turn
        day = int(day)
        turn = int(turn)
    ...
    def subtract_turn(self):
        # Spend one turn and start a new day when necessary.
        global turn
        global day
        turn = int(turn)
        day = int(day)
    ...
    def save_turn(self):
        # Persist the number of turns remaining.
        global turn
        with open("turn.txt", "w") as f:
            f.write(str(turn))
 ...
 # Tkinter screens, navigation, controls, and status displays.
class GUI:
    def __init__(self, root):
    # Create the game logic and bind its state to Tkinter variables.
        ...
        # Build every playable, win, and tutorial screen once.
        self.frames = {}
        ...
        self.frames['Tutorial1'] = self.create_tutorial_frame(
            'You got yourself in generational debt...',
            'Idk how but you owe Jaiden...',
            'Tutorial2'
        )
        self.frames['Tutorial2'] = self.create_tutorial_frame(
            '$1000000000!!? Yeah, good luck with that gang.',
            'Thats like alotta money.',
            'Tutorial3',
            'Tutorial1'
        )
        self.frames['Tutorial3'] = self.create_tutorial_frame(
            'Just gamble ur life saving away, maybe ull get lucky!',
            'Use the slot machines to earn money and pay off the debt.',
            'Tutorial4',
            'Tutorial2'
        )
        self.frames['Tutorial4'] = self.create_tutorial_frame(
            'Turns',
            'You have 10 turns each day. Working or rolling uses one turn.\nWhen your turns run out, the next day begins.',
            'Tutorial5',
            'Tutorial3'
        )
        self.frames['Tutorial5'] = self.create_tutorial_frame(
            'Rolling',
            'Choose a machine, enter a bet, and press Roll.\nMatching symbols and special symbols can win money.',
            'Tutorial6',
            'Tutorial4'
        )
        self.frames['Tutorial6'] = self.create_tutorial_frame(
            'How to roll',
            '1. Choose a machine.\n2. Enter a bet within its minimum and maximum.\n3. Press Roll and hope for a win.\n4. Use your money to pay off the debt.',
            'TutorialDone',
            'Tutorial5'
        )

        # First-time players begin with the tutorial; returning players use the menu.
        if tutorial_complete:
            self.show_frame('Mainframe')
        else:
            self.show_frame('Tutorial1')
    
    def show_frame(self, name):
        # Raise the requested screen above the other screens.
        frame = self.frames[name]
        frame.tkraise()

    def create_tutorial_frame(self, title, text, next_frame, previous_frame=None):
        # Build one reusable tutorial page with optional Back and Next controls.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
        frame.columnconfigure([0, 1], minsize=200, weight=1)
        frame.rowconfigure([0, 1, 2], minsize=100, weight=1)

        Label(frame, font=FONT_MAIN_TITLE, text=title).grid(row=0, columnspan=2, padx=20, pady=20, sticky='nsew')
        Label(frame, font=FONT_HEADING, text=text, justify='center').grid(row=1, columnspan=2, padx=20, pady=20, sticky='nsew')
        if previous_frame is not None:
            Button(frame, text='Back', bg='white', font=FONT_HEADING, command=lambda: self.show_frame(previous_frame)).grid(row=2, column=0, padx=10, pady=20, sticky='nsew')
        next_command = self.finish_tutorial if next_frame == 'TutorialDone' else lambda: self.show_frame(next_frame)
        Button(frame, text='Next', bg='white', font=FONT_HEADING, command=next_command).grid(row=2, column=1 if previous_frame is not None else 0, padx=10, pady=20, sticky='nsew')

        return frame

    def finish_tutorial(self):
        # Save tutorial completion and send the player to the main menu.
        with open("tutorial_complete.txt", "w") as f:
            f.write("1")
        self.show_frame('Mainframe')

    def show_win_frame(self):
        # Update the final day count and show the win message.
    ...
    def create_win_message_frame(self):
        # Build the win screen with the image and completion message.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
    ...
    def create_win_choice_frame(self):
        # Build the continue-or-quit screen shown after winning.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
        frame.columnconfigure([0, 1], minsize=200, weight=1)
    ...
    def create_odds_panel(self, parent, machine_number):
        # Create the black odds panel shared by machine screens.
    ...
    def create_main_frame(self):
        # Build the title screen and Play button.
        frame = Frame(self.container)
        frame.grid(row=0,column=0,sticky='nsew')
        frame.columnconfigure([0, 1], minsize=200, weight=1)
        frame.rowconfigure([0, 1], minsize=100, weight=1)
    ...
    def create_work_frame(self):
        # Build the work, debt payment, navigation, and status screen.
        frame = Frame(self.container)
        frame.grid(row=0,column=0,sticky='nsew')
    ...
    def create_machine1_frame(self):
        # Build the first slot machine screen.
        # Machine 1 Frame
        global machines
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
    ...
    def create_machine2_frame(self):
        # Build the second slot machine screen.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
        frame.columnconfigure([0, 1, 2, 3, 4, 5, 6], minsize=50, weight=1, uniform='machine')
        frame.rowconfigure([0, 1, 2, 3, 4, 5], minsize=50, weight=1, uniform='machine')
    ...
    def create_machine3_frame(self):
        # Build the third slot machine screen.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
        frame.columnconfigure([0, 1, 2, 3, 4, 5, 6], minsize=50, weight=1, uniform='machine')
        frame.rowconfigure([0, 1, 2, 3, 4, 5], minsize=50, weight=1, uniform='machine')
    ...
    def create_machine4_frame(self):
        # Build the fourth slot machine screen.
        frame = Frame(self.container)
        frame.grid(row=0, column=0, sticky='nsew')
        frame.columnconfigure([0, 1, 2, 3, 4, 5, 6], minsize=50, weight=1, uniform='machine')
        frame.rowconfigure([0, 1, 2, 3, 4, 5], minsize=50, weight=1, uniform='machine')
...
# Create the application window and start Tkinter's event loop.
root = Tk()
