...
machine_odds = {
    "1": {"1": [500, 400, 99, 1],
            "2": [10, 500, 300, 189, 1], # All chances are based on .../1000
            "3": [199, 400, 400, 1],
            "4": [500, 499, 1]}
}
...
class Logic:
    ...
    def roll(self, entry, currency, machine_number, slot1, slot2, slot3):
        ...
        if status == True:
            ...
            winning = random1[0]
            if random1[0] in ['x10', 'x2000', 'GoodJaiden', 'ULTRAJaiden']:
                player_money += int(entry)*machine_winning[machine_number][random1[0]]
                currency.configure(text=player_money)
            elif random2[0] in ['x10', 'x2000', 'GoodJaiden', 'ULTRAJaiden']:
                player_money += int(entry)*machine_winning[machine_number][random2[0]]
                currency.configure(text=player_money)
            elif random3[0] in ['x10', 'x2000', 'GoodJaiden', 'ULTRAJaiden']:
                ...
                player_money += int(entry)*machine_winning[machine_number][winning]
                currency.configure(text=player_money)
...
class GUI:
    ...
    def create_machine1_frame(self):
        ...
        chances_lbl = Frame(frame.odd, font=FONT_DEFAULT, text='')
